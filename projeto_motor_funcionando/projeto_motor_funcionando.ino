...
String getPage() {
  ...
  <head>
    ...
    <style>
      ...
      form, canvas, #serialLog, #manualControl {
        margin-top: 20px;
        background: #fff;
        padding: 15px;
        border-radius: 8px;
  ...
  <body>
    ...
    <script>
      ...
      const angleChart = new Chart(angleCtx, {
        type: 'line',
        data: { labels: [], datasets: [{ label: 'Ângulo (°)', borderColor: 'blue', data: [], tension: 0.3 }] },
        options: { animation: false }
      });

      const pwmChart = new Chart(pwmCtx, {
        type: 'line',
        data: { labels: [], datasets: [{ label: 'PWM (us)', borderColor: 'green', data: [], tension: 0.3 }] },
        options: { animation: false }
      });
      ...
      ws.onmessage = (event) => {
        ...
        // Limita o histórico em 100 linhas
        const lines = logDiv.innerText.split('\n');
        if (lines.length > 100) {
          logDiv.innerText = lines.slice(lines.length - 100).join('\n');
        }
        logDiv.scrollTop = logDiv.scrollHeight;
      };

      function toggleManual() {
        fetch('/toggleManual').then(() => {
...
void handleSet() {
  if (server.hasArg("kp")) {
    ...
    if (newKp >= 0 && newKp <= 10) Kp = newKp;
  }
  if (server.hasArg("ki")) {
    float newKi = server.arg("ki").toFloat();
    if (newKi >= 0 && newKi <= 5) Ki = newKi;
  }
  if (server.hasArg("kd")) {
    float newKd = server.arg("kd").toFloat();
    if (newKd >= 0 && newKd <= 10) Kd = newKd;
  }
  server.send(200, "text/html", getPage());
}
...
void setup() {
  ...
  Serial.println("ESPERANDO O MOTOR FICAR PRONTO...");
